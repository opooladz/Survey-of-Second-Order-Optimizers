...
def LM(model,loss,n_iter=200):
    ...
    # for i, (data, target) in enumerate(torch_dataset):
        ...
        prev_loss = torch.mean(diff ** 2)#.item()            

        # gradients=torch.autograd.grad(loss_out, model.parameters(), create_graph=True)
        # Caluclate Jacobian 
        J = jacobian(diff,model.parameters(), create_graph=True, retain_graph=True)
        ...
        g_vector = torch.matmul(J.T,diff).unsqueeze(1)
        del J
        dx=-1*torch.matmul(torch.inverse((alpha*torch.eye(Hessian.shape[-1]).cuda('cuda:0')+Hessian)),g_vector).detach()
        # dx=-1*torch.matmul(inv_block_gpu((alpha*torch.eye(Hessian.shape[-1]).cuda('cuda:0')+Hessian),4),g_vector).detach()
        ...
        # dx=-1*torch.matmul(inv_block_gpu((alpha*torch.diag(Hessian).cuda('cuda:0')+Hessian),10),g_vector).detach()
        ...
        diff2 = (out-target).squeeze()
        loss_out=torch.mean(diff2**2)
        # print(prev_loss)
        # print(loss_out)
...
data, target = Variable(x).to('cuda:0'), Variable(y).to('cuda:0')
...
loss_hist=LM(net,loss_func,n_iter=200)
